...
class Metrics():
  ...
  def __init__(self,model,data,Ndraws,filename,Ncoords):
    ...
    self.data_val = data_[:Ncoords,:,0].reshape(Ncoords,data_.shape[1],1)
    self.data_test= data_[:Ncoords,:,1:]
    self.data=data[0]
    self.model=model
    self.Ndraws=Ndraws
    self.params=params_
  ...
  def multi_ef_test(self,test_cones,params,Ndraws=1000,Ncones_test=101,rng=60):
  ...
  def multi_ef(self):
    self.ef_val=vmap(self.multi_ef_mapped,in_axes=(0,0,0))(self.data_val[:,:-1,:],self.params[:self.data_val.shape[0]],jnp.arange(self.data_val.shape[0]))
    self.ef_test=vmap(self.multi_ef_mapped,in_axes=(0,0,0))(self.data_test[:,:-1,:],self.params[:self.data_val.shape[0]],jnp.arange(self.data_test.shape[0]))
