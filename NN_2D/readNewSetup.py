...
from SGEmultiEC import error,prior,truePAC,LipC
...
pathT='results/s_data/tables/'
pathF='results/s_data/figures/'
...
day='10_20'
file_name= day+'_depth_relu_std'+preTlabel#full_tanh_new_setup,full_relu_new_setup
...
for current_id in reversed(range(len(datasetsM))):
    ...
    for i,arch in enumerate((archs)):#reversed 
            fVal=open(pathT+'Table_validation_'+figure_name+'_'+datasetsM[current_id]+str(dim([inp,*archs[i]]))+'_'+'.txt','w')
            ...
            fTest=open(pathT+'Table_TestTrain_'+figure_name+'_'+datasetsM[current_id]+str(dim([inp,*archs[i]]))+'_'+'.txt','w')
            ...
            for pir in piRescaling:
                  ...
                  pathPrior=pathF+datasetsM[current_id]+'/'+'prior'+str(pir)+'/'+day+'/'+str(dim([inp,*arch])) +'/'
                  ...
                  for m in range(len(m_batches)):  
                    hf=h5py.File(path+'prior' + str(pir) +'var/' +file_name +str(dim([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches[m]) +'_Epoch_'+str(Nepochs)+'.h5','r')
                  ...
                  plt.savefig(pathPrior+figure_name+'_fullEpochslog_'+str(dim([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches[m]) +'_Epoch_'+str(Nepochs)+'.png')
                  plt.close()
                  ...
                  plt.savefig(pathPrior+figure_name+'_lastEpochslog_'+str(dim([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches[m]) +'_Epoch_'+str(Nepochs)+'.png')
                  plt.close()
