...
for current_id in reversed(range(len(datasetsM))):
    ...
    for i,arch in enumerate((archs)):#reversed 
            ...
            if not os.path.exists(pathF):
                      os.makedirs(pathF)
            fVal=open(pathT+'Table_validation_'+figure_name+'_'+datasetsname_short[current_id]+str(archs[i])+'.txt','w')
            fVal.write('Train Err. & CRPS & RMSE\n\n')

            fTrain=open(pathT+'Table_training_'+figure_name+'_'+datasetsname_short[current_id]+str(archs[i])+'.txt','w')
            fTrain.write('Training E. & Lin. PAC & True PAC\n\n')
                            
            
            
            fTest=open(pathT+'Table_TestTrain_'+figure_name+'_'+datasetsname_short[current_id]+str(archs[i])+'.txt','w')
            ...
            for pir in piRescaling:
                  ...
                  for m in range(len(m_batches)): 
                    ...
                    print(np.round(min_error_sup,decimals=4),'&',np.round(np.mean(crps),decimals=4),'&',np.round(np.mean(np.sqrt(rmse)),decimals=4),'&',min_it_E,file=fVal)    
                  ...
                  savename = pathF+"E"+datasetsname_short[current_id]+ 'prior' + str(pir)+folder_day+str(arch)+figure_name +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches[m]) +'_Epoch_'+str(Nepochs)+'.png'.replace("/","-")
