...
archs=[[2,3,1]]#[[30,30,1],[100,100,1],[300,300,1]]  ##,,[300,300,1],[100,100,1],
  ...
  for i,arch in enumerate(archs): # reversed
    ...
    for current_id in range(len(datasetsM)):
      ...
      for k,pir in enumerate(piRescaling):
        ...
        Optimization(file_m,Z,x_size,boolean,rescaling,eps,delta,data,inp,p,c,arch,dimComp([inp,*arch]),Ndraws,m_batches,m_test,Ncoords,shard_size[i], lr, epochs=Epochs, piScaling=pir, bounded=False)
        file_.close()