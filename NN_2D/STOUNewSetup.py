...
class STOU:
    def __init__(self,x_position,data,A_estimated,c_estimated,archs,N,N_last_cones=50,Ncones=1,a=1,p=1,num_realizations=20,h_t=0.05): #,model
        """
        ...
        #time.sleep(5)  
        self.x_position=np.array([c_estimated*p])
    ...
    def set_model_for_p_value(self,data,arch,dim,mask,loss,eps,p=1,num_realizations=2,mean=0.0,var=1,num=100):
    ...
    def get_cone_shiftJ(self):
        ...
        for t in reversed(range(1,self.p+1)):
            ...
            #print(jnp.array([[v, - (t )] for v in range(int(-jnp.ceil(self.c_*t).astype('int16')), int(jnp.ceil(self.c_*t).astype('int16')) + 1)]))
            coord.append(jnp.array([[v, - (t )] for v in range(int(-jnp.ceil(self.c_*t).astype('int16')), int(jnp.ceil(self.c_*t).astype('int16')) + 1)]))  
            #zcoord.append(jnp.array([[v, - (t + 1)] for v in range(int(-jnp.ceil(self.c_*self.p).astype('int16')*(t + 1)), int(jnp.ceil(self.c_*self.p).astype('int16')*(t + 1)) + 1)]))
    ...
    def get_coneJ(self,data,sizeData,preT=False):
        ...
        #print('cone',cone_coordinates[0,0])
