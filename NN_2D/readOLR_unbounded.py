...
if a_val[0]==18:
  ...
  m_batches = [36]
Ncoords=8+2*int(np.floor(hatc))#10
Ncones_test=19
...
for i,arch in enumerate((archs)):#reversed  
      ...
      for k,pir in enumerate(piRescaling):
            ...
            for m in range(len(m_batches)): 
              path_and_file_name = os.path.join(
                  path,
                  'prior' + str(pir) + 'var',
                  day+str(arch) +'_' +str(datasets[current_id]) +'_a' +str(a_val[0]) +'_pir' +str(piScalingLabel[k]) +'_m' +str(m_batches[0]) +'_Epoch_'+str([Nepochs])+'.h5'
              )
              try:
      ...
      print("& $",arch[0],'^2$ & $\\mathcal{N}(0,1/',best_prior,')$ &',m_batches[0],'&',np.round(KL_train_best_iter,decimals=4) ,'&',np.round(Liph_train_best_iter,decimals=4),'&',np.round(best_prior_emp_train_error,decimals=4),'&', np.round(best_prior_best_train_error,decimals=4),'&',1,'&',np.round(best_crps_val,decimals=4),'&',np.round(best_rmse_val,decimals=4),'&',Ncones_test,'&',np.round(corr_crps_test,decimals=4),'&',np.round(corr_rmse_test,decimals=4),"\\"+"\\"+"\n", file = fMMAF)  
