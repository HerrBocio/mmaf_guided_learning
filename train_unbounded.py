...
def coordit_unbounded(model,optimizer,theta,VarZtrx,params,batch,opt_state,rng):
  ...
  m = batch[-1].shape[0]
  kl_mapped = lambda beta: target_function_unbounded_KL(model.pi_params,beta,m)
...
def train_unbounded(config):
      ...
      for epoch in trange(1,config.data.epochs+1, desc='epochs', colour='green'):
          ...
          for ds_idx,data_shard in enumerate(embedding.clean_data):
              ...
              for el in jnp.arange(config.num_realizations//config.shard_realization):
