...
day= '12_18' # '09_25' # 
pathT= 'results/snmall_nets/tables/'+day+'/' # 'results/err_corr/a_4/tables/'+day+'/'
pathF= 'results/small_nets/figures/'+day+'/' # 'results/err_corr/a_4/figures/'+day+'/'
#path='/afs/tu-chemnitz.de/project/calibration/paper_simulations/m_wise_epoch_wise/'

create_folder(pathT)
create_folder(pathF)

path='/LOCAL/prol/natale/results/'
...
pretraining=False
if pretraining:
  Epochs=range(1,151)#,61) #
  Nepochs=150 #60#len(Epochs)
  preTlabel='_preT'
else:
  Epochs=range(1,61) #,31) #
  Nepochs=60 # 30#len(Epochs)
  preTlabel=''
...
constants_train=2*np.log(1./0.025)/np.sqrt(m_batches) + eps**2/(2*np.sqrt(m_batches))
...
for current_id in reversed(range(len(datasetsM))):
    ...
    for i,arch in enumerate((archs)):#reversed 
            fVal=open(pathT+'Table_validation_'+figure_name+'_'+datasetsM[current_id]+str(dimComp([inp,*archs[i]]))+'_a'+str(a_val[current_id])+'.txt','w')
            fVal.write('Train Err. & CRPS & RMSE\n\n')

            #fTrain=open(pathT+'Table_training_'+figure_name+'_'+datasetsM[current_id]+str(dim([inp,*archs[i]]))+'_'+'.txt','w')
            #fTrain.write('Training E. & Lin. PAC & True PAC\n\n')
                            
            
            fTest=open(pathT+'/'+'Table_NewHorizon_'+figure_name+'_'+datasetsM[current_id]+str(dimComp([inp,*arch]))+'_a'+str(a_val)+'.txt','w')
            #fTest.write('KL \t\t Chi2 \t\t pac lin \t\t pac true \t\t crps \t\t rmse \t\t min. it \n\n') #Test Err. &
	    fTest.write('crps \t\t rmse')
            ...
            for pir in piRescaling:
                  ...
                  if pretraining:
                    ...
                    piMean= dist_sample(initParams,seed=jax.random.key(0))
                  ...
                  hf=h5py.File(path+'prior' + str(pir) +'var/'+day +file_name +str(dimComp([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val) +'_pir' +str(pir) +'_m' +str(m_batches) +'_Epoch_'+str(Nepochs)+'.h5','r')
                  m_g=hf.get('m'+str(m_batches))                    
                  print(hf.keys())
                  #print(m_g.keys())
                  ...
                  truePacFunTrain=jax.vmap(lambda beta,gamma: truePAC(beta,piParams,eps,Lip,delta,inp,m_batches,alph,p,d,arch,gamma),in_axes=(0,0))
                  ...
                  print('chi2',Chi2)
                  pac_true_min_test =jnp.mean( true_boundTest+emp_min)
                  pac_true_min_train= jnp.mean(true_boundTrain+emp_min_train)
                  print('true pac',pac_true_min_train)   
                  ##### TRAIN #####
                  ...
                  rmse=[]
                  for n1 in range(8):
                    crps_coord=crps_univ_rank_mapped(b_validation[n1],m_e_f_validation[n1,:,0])
                    crps.append(crps_coord)
                    rmse_coord=rmse_univ(b_validation[n1],m_e_f_validation[n1,:,0])
                  ...
                  for i in range(p*c,data_test.shape[0]-p*c):
                     coord_e_f=multi_ef_test(data_test[i-p*c:i+p*c+1,-(Ncones_test-1)*a_val[current_id]-2:-(Ncones_test-1-horizon)*a_val[current_id]:a_val[current_id]],best_params[i-p*c,:,:],inp,arch,mask,Ndraws,Ncones_test-1,rng)
                     #print(i,coord_e_f.shape)
                  ...
                  for n1 in range(8):
                    ...
                    crps.append(np.mean(crps_coord))
                    rmse.append(np.mean(rmse_coord))
                  ...
                  print(np.round(np.mean(crps),decimals=4),'&',np.round(np.mean(rmse),decimals=4), file=fTest) 


                  np.round(KL,decimals=4),'&',np.round(Chi2,decimals=4),'&' ,np.round(min_error,decimals=4),'&', np.round(pac_true_min_train,decimals=4),'&'
                  ...
                  plt.plot(np.arange(Nbatches,len(pac_obj)+1),pac_obj[Nbatches-1:]+empirical_obj[Nbatches-1:],linewidth=1,color='red')
  
                  plt.plot(np.arange(Nbatches,len(bound_train)*Nbatches+1,Nbatches),np.array(bound_train)+np.array(train_errors),linewidth=1,color='navy')

                  #plt.plot(np.arange(len(bound_train)*Nbatches+1,50*Nbatches+1,Nbatches),np.array(bound_train)+np.array(train_errors),linewidth=1,color='navy')
                  
                  #plt.yscale("log")
                  plt.xlabel("epochs")
                  plt.xticks(range(Nbatches,len(bound_train)*Nbatches+1,Nbatches),list(range(1, len(bound_train)+1)))
                  #plt.plot(np.arange(1,len(pac_obj)+1),np.ones(len(pac_obj))*eps,color='seagreen',linestyle='--')

                  #plt.ylabel("obj function value")
                  plt.legend(['Target function','MC Target function'])#,'vacuity threshold'])
                  plt.tight_layout()
                  plt.savefig(pathPrior+figure_name+'_overlap_'+str(dimComp([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches) +'_Epoch_'+str(Nepochs)+'.png')

                  plt.savefig(pathPrior+figure_name+'_overlap_'+str(dimComp([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches) +'_Epoch_'+str(Nepochs)+'.eps',format='eps')
                  ...
                  plt.savefig(pathPrior+figure_name+'_fullEpochslog_'+str(dimComp([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches) +'_Epoch_'+str(Nepochs)+'.png')
                  plt.close()
                  ...
                  plt.savefig(pathPrior+figure_name+'_lastEpochslog_'+str(dimComp([inp,*arch])) +'_' +str(datasetsM[current_id]) +'_a' +str(a_val[current_id]) +'_pir' +str(pir) +'_m' +str(m_batches) +'_Epoch_'+str(Nepochs)+'.png')
                  plt.close()
                  '''
fVal.close()              
fTest.close()
