...
#rewrite class wrt to functional syntax
  ...
  def sample(self, size):
    ...
    sam=jax.vmap(lambda k : jax.random.normal(k, shape=sample_shape) * lax.stop_gradient(self._scale) + (self._mean) )(subkeys)
...
def pacApprox(params,piParams,eps,delta,a_p_c,a,m,alph,lambda_,p,dim,arch):
    ...
    bb=0
    rhoParams=[lax.stop_gradient(params[0]),params[1]]#jnp.diag(params[1])]
...
#@jit
def OptSGD(Z,x1_size,x2_size,loss,eps,delta,data,inp,p,c,arch,dim,Ndraws,Ncones,Ncoords,shard_size,lr,rhoScaling,slope=0,q=0,piScaling=1):
    ...
    rhoParams=[jnp.ones(dim)/20,jnp.ones(dim)/(Ncones+1)]#
    if piScaling!=1:
      print('fixed setup')
    ...
    def coordit(it_coord,it_params,opt_state,batch,Bsize,slope,q,rng=rng):
        ...
        def mesc(ind):
          ...
          return [m,n]
        window_mapped=time_mapped[:,*mesc(it_coord)]#print('coord',x_coord)
        #print('shape',window_mapped.shape)
        #window_mapped=jnp.reshape(window_mapped,(len(it_coord),Bsize))
        #print('shape',window_mapped.shape)
        #print(last_cone.shape)
        
        cone_mapped=  last_cone[:,*mesc(it_coord)]#jax.lax.map(last_cone_slicing,(it_coord))
        #print(cone_mapped)
        #cone_mapped=jnp.reshape(cone_mapped,(len(it_coord),Z.a ))
        #print('shape',cone_mapped.shape)
        Acones,bcones=Z.get_coneJ_3d((window_mapped))#,sizeData=time_windows[batch].shape[0])
        ...
        A,b=Z.get_coneJ_3d(cone_mapped)
    ...
    list_shards= [jnp.array(
                  [
                   jnp.array([jnp.arange(c_coord_x1-1,c_coord_x1+1+1),jnp.arange(c_coord_x2-1,c_coord_x2+1+1)]) 
                   for c_coord_x1 in range(element1,element1+shard_size)   
                   for c_coord_x2 in range(element2,element2+shard_size) 
                  ]
                  )
                   for element1 in range(1,x1_size,shard_size) for element2 in range(1,x2_size,shard_size)]
    ...
    #here starts a new batch init
        ...
        for ls_i in range((x1_size-2*p)//shard_size*(x2_size-2*p)//shard_size):
        ...
        if Xtesting > .2:
          ##print('convergence after ',batch,' iterations')
          break
    print('convergence after ',batch,' iterations with p-value ',Xtesting) 
    '''
    ...
    for ls_f in range((x1_size-2*p)//shard_size*(x2_size-2*p)//shard_size):   
    ...
    #print(finalPit)
    '''
    return [output,params,[0],pit_vals,batch,kstesting]
