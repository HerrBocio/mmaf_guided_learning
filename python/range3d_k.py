...
# from STOUleo import set_model_for_p_value, infer_beta_non_averaged
...
def predict_slice(Z, data, x_size, y_size, loss, p, c, Ndraws, eps, mean=0.0, var=1):
    ...
    for i in range(x_size):
        ...
        y_models = []
        #print(x_size, y_size)
        for j in trange(y_size, desc='posterior', colour='red'):  # ,desc='posterior',colour='red'): #making the Ndraws
...
def predict_k_slice(Z, data, k, x_size, y_size, loss, p, c, Ndraws, eps, mean=0.0, var=1):
    """
    Funzione che ti predice il punto del cono piu' nuovo
    usando tutto il cono nuovo (tranne la punta in cui vogliamo fare la previsione)
    """
    ...
    # t in [p-1, p-2, ..., 0]
    for t in reversed(range(p+2*k-1)):  # self.p = p
        # aggiunge alla fine di coord il vettore dentro tf.constant()
        coord.append(tf.constant([[- (t+1), v, u] for v in range(-int(np.floor(c*(t + 1)/np.sqrt(2))+1), int(np.floor(c*(t + 1)/np.sqrt(2))+1) + 1)
                     for u in range(-int(np.floor(c*(t + 1)/np.sqrt(2))+1), int(np.floor(c*(t + 1)/np.sqrt(2))+1) + 1)]))
        # dentro tf.cponstant() costruisco la matrice di righe v in range(..) e colonne -(t+1)
    ...
    for i in trange(x_size, desc='posterior', colour='red'):
        ...
        for j in range(y_size):  # ,desc='posterior',colour='red'): #making the Ndraws
            ...
            # print("exceeds")
            # if it's outside the cone then skip
            if (i < c*(p+2*(k-1)) or j < c*(p+2*(k-1))) or (i > x_size-1-c*(p+2*(k-1)) or j > y_size-1-c*(p+2*(k-1))):
                # print("if true")
                # p_values.append(-1)
                # print("    p:",p_values[-1])
                y_models.append(np.nan)
            ...
            # ,mean=mu[i],var=sigma[i])
            Z.set_model_for_p_value(
                p+2*k-1, c, loss, eps, mean=mean, var=var)
    ...
    for r in trange(Ndraws, position=0, leave=True, desc="r", colour='green'):
        ...
        for i in range(x_size):
            ...
            for j in range(y_size):
                ...
                cone_ends_coordinates = tf.expand_dims(
                    cone_end_coordinates, 0)

                cone_coordinates = cone_ends_coordinates+coord[0, 0, :, :]

                extreme = 0
                for el in range(k, p+2*k-1):
                    extreme = extreme + (int(2*np.floor(c*el))+1)**2
                    # print('interntempo', extreme)

                A = tf.gather_nd(
                    data, cone_coordinates[:extreme, :])

                # print('MATRICDE DI INPUT: ', A)
                # print('coefficiente moltiplicativo della Gibbs:',
                #      x_models[i][j][r, :extreme])
                # print('NUMERO DI INPUT:', extreme)

                value = tf.reduce_sum(
                    A*x_models[i][j][r, :extreme])+x_models[i][j][r, -1]
