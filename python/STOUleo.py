...
def modello_teoricoSTOU(h, theta):
    return 2*np.pi*theta[2]*theta[1]**2 *(1 - np.exp(-theta[0]*h))
# MODELLO TEORICO MSTOU
# theta = [alpha, c, Var, beta, a]
def modello_teoricoMSTOU(h, theta):
    return (np.pi*theta[2]*theta[1]**2*gamma(theta[0]-3)*theta[3]**3)/(4*gamma(theta[0]-1))*(1 - (1 + theta[3] * h)**(theta[0]-3))
...
class STOU:
    ...
    def __init__(self, x_position, data, lambda_, c_, num_realizations=2000, h_t=1):
        """
    ...
    def get_cone(self, x):
        ...
        """
        # x_size, N = data.shape
        # print("p: ",self.p, "a: ", self.a, "N:", self.N)
        # considero le punte dei coni, occhio che self.N e' escluso
        # print(self.p)
        cone_ends = tf.range((self.N-self.a) %
                             self.a + self.p, (self.N-self.a), self.a)
        # print(f"1) cone_ends = {cone_ends}")

        cone_ends_shape = ((self.N-self.a) - 1 -
                           (self.N-self.a) % self.a) / (self.a)
        ...
        b = tf.gather_nd(self.data[:-self.a, :, :], cone_ends_coordinates)
        ...
        A = tf.gather_nd(self.data[:-self.a, :, :], cone_coordinates)
    ...
    def helper1(self, x):
        ...
        return - self.lambda_*self.h_t*(x-self.p) - np.log(x/(2*self.N))

    def helper2(self, x):
        return - self.lambda_*np.log(self.h_t*(x-self.p)) - np.log(x/(2*self.N))
