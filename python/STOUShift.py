...
#Forza il sistema ad utilizzare le CPU invece delle GPU
os.environ["CUDA_VISIBLE_DEVICES"]="-1"



#with open('SimsOutput.txt','w') as outfile1:


def get_loss_function(A,b,loss,m=1,eps=2.99):
    fun_sq = lambda beta: (tf.einsum("nm,bm->bn", A, beta[:, :-1]) + tf.expand_dims(beta[:, -1], 1) - b)**2 #sq LOSS!
    
    fun_abs = lambda beta: np.abs(tf.einsum("nm,bm->bn", A, beta[:, :-1]) + tf.expand_dims(beta[:, -1], 1) - b) #abs LOSS!
    
    fun_b = lambda x: (tf.cast(x <= eps, dtype=tf.float64) * x + tf.cast(x > eps, dtype=tf.float64) * eps) #eps-bounded function
    
    
    if loss.find('absUnB')+1: #loss=='absUnB':
        r_eps = lambda beta: tf.reduce_mean(fun_abs(beta), axis=1)#*m#ABS UNBOUNDED!
    elif loss.find('sqUnB')+1: #loss =='sqUnB':
        r_eps = lambda beta: tf.reduce_mean(fun_sq(beta), axis=1)#*m#SQUARE UNBOUNDED!
    elif loss.find('absB')+1: #loss =='absB' :
        print('res ',loss.find('absB')+1)
        r_eps = lambda beta: tf.reduce_mean(fun_b(fun_abs(beta)), axis=1)#*m#ABS BOUNDED!
       	#print("in abs")
    elif loss.find('sqB')+1: #loss =='sqB':
        r_eps = lambda beta: tf.reduce_mean(fun_b(fun_sq(beta)), axis=1)#*m#SQUARE BOUNDED!
    #else:
    	#break
    return r_eps


def get_posterior_distribution(A,b,loss,m,eps=2.99,l=tf.constant(2.0,dtype=tf.float64), num_realizations=10000,mean=0.0,var=1):
    """
    Funzione che restituisce 
    un elemnto di classe Sampler
    """
    r_eps = get_loss_function(A,b,loss,eps=eps)#,m PUT M BACK!!!!
    prior = get_gaussian_sampler(A.shape[1]+1, mean=mean, var=var)
    #print(b.shape,A.shape)
    g_unormalized = lambda beta : tf.math.exp(-tf.sqrt(l)*r_eps(beta))#*m
    realization = prior(num_realizations)
    c = tf.reduce_mean(g_unormalized(realization))
    #print("n_c: ",c)
    g_normalized = lambda beta: g_unormalized(beta)/c
...
class STOU:
    def __init__(self,x_position,data,A_,c_,num_realizations=2000,h_t=0.05): #variogram left out: needed?
        """
        ...
        self.x_position = tf.constant(x_position,shape=(1,),dtype=tf.int32)
    ...
    def get_cone_shift(self):
        ...
        for t in reversed(range(self.p)): #self.p = p
            ...
            coord.append(tf.constant([[v, - (t + 1)] for v in range(-(t + 1), t + 1 + 1)]))
    ...
    def get_cone(self,x):
        ...
        #Dubbio: non ho capito perche' riscala il vettore cone_ends
        #e perche' il passo e' p+a+1 invece di a
        #ma poi, non basterebbe definire:
            #cone_ends = tf.range(self.p, self.N, )
        
        #cone_ends = cone_ends - tf.reduce_max(cone_ends) + self.N-1

        cone_ends_shape = (self.N - 1 - self.p) / (self.p + self.a + 1)
        #print("cone ends shape: ",cone_ends_shape)
        
        #cast a tensore di numeri interi e non a semplice floor
        cone_ends_shape = tf.cast(tf.floor(cone_ends_shape) + 1, dtype=tf.int32)
        #print("cone ends shape: ",cone_ends_shape)

        #MEMORIA MASSACRATA
        x = tf.expand_dims(tf.expand_dims(x,1),1)
        cone_ends = tf.expand_dims(tf.expand_dims(cone_ends,1),0)
        #sulla prima e terza colonna questo comand tf.broadcast_to() non fa 
        #nulla mentre la seconda, che sarebbero le righe dele matrici, 
        #ne mette un numero uguale a quello di cone_ends_shape
        x = tf.broadcast_to(x,[x.shape[0],cone_ends_shape,1])
        ...
        cone_ends = tf.broadcast_to(cone_ends, x.shape)
        #unisco le righe di x con quelle di cone_ends
        #esempio: 
            #x = x = tf.constant([[[4, 4, 4, 4], [3, 3, 3, 3]], [[4, 4, 4, 4], [3, 3, 3, 3]]])
        ...
        cone_ends_coordinates = tf.concat( (x, cone_ends),axis=2)
        
        #estrai da self.data i valori di indice cone_ends_coordinates
        b = tf.gather_nd(self.data, cone_ends_coordinates)
        #print("b shape: ",b.shape)
        
        #espando in posizione 2 perche' voglio sommare le righe delle matrici 
        #ricorda come e' fatto self.cone_shift
    ...
    def set_model_for_benchmark(self,p):
        ...
        design_matrix = np.concatenate((self.A.numpy(),np.ones((self.A.shape[0],1))),axis=1)
        mat = scipy.linalg.pinv(np.matmul(np.transpose(design_matrix), design_matrix))
        ...
        self.beta_linear_modelA = minimize(cost_f,np.ones((self.beta_linear_modelS.shape[0],),dtype=np.float64),args=(design_matrix,b)).x
    ...
    def set_model_for_p_value(self,p,c,loss,eps=2.99,mean=0.0,var=1):
        ...
        self.r_eps = get_loss_function(self.A, self.b, loss, self.m, eps=eps)
        #qui sotto definisco l'attributo self.sampler usando una funzione definita
        #che restituisce un elemento di classe Sampler
        self.sampler = get_posterior_distribution(self.A, self.b, loss, self.m, eps=eps, l=self.l, num_realizations=100000,mean=mean,var=var)
    ...
    def stouPar(self,tau,c):
        ...
        r_y = np.ones((1,3),dtype=np.float64)*tau**2*c*tf.math.exp(-3*self.lambda_)/c*self.lambda_*(1+1/(2*self.lambda_))
        
        for i in range(R_Y.shape[1]):
            R_Y[i,i]= 2*tau*2*c/(4*self.lambda_**2)
        
        aux = tau**2*tf.math.exp(-self.lambda_/c)/(2*self.lambda_) #just a factorized term
    ...
    def post_predictive(self,tau=1,c=1,mu=0):
        ...
        *Per d>1 sei ingulat
        ...
        for t in reversed(range(self.p)):
            coord.append(tf.constant([[v, - (t + 1)] for v in range(-c*(t + 1), c*(t + 1) + 1)]))
        coord = tf.concat(coord, axis=0)
        coord = tf.expand_dims(tf.expand_dims(coord, 0), 0)
        ...
        for x_coord in range(self.data.shape[0]):
            ...
            cone_end_coordinates = tf.constant([x_coord, self.data.shape[1]], dtype="int32")
            ...
            A = tf.gather_nd(self.data, cone_coordinates) 
            #print(A.shape)
            mean = 2*c*mu*self.lambda_**2 + np.matmul(np.matmul(r_y,scipy.linalg.pinv(R_Y)),(np.transpose(A)-2*c*mu/self.lambda_**2))
            #print(np.mean(mean))
            pred_mean.append(np.mean(mean))
            var = c*tau**2/(2*self.lambda_**2)*(1-np.matmul(r_y,np.matmul(scipy.linalg.pinv(R_Y),np.transpose(r_y))))
            #print(np.mean(var))
